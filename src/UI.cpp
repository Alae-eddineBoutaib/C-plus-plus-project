...
{
    // Deep red/brown gradient background
    DrawRectangleGradientV(0, 0, screen_width, screen_height, Color{44, 29, 24, 255}, Color{15, 10, 8, 255});
    draw_centered_text("KEYBINDS", 70, 58, RAYWHITE);

    // I increased the height (from 280 to 420) and moved it up slightly to fit more lines
    Rectangle box = {screen_width / 2.0f - 300.0f, 180.0f, 600.0f, 420.0f};
    DrawRectangleRounded(box, 0.15f, 8, Fade(BLACK, 0.45f));
    DrawRectangleLinesEx(box, 2.0f, Fade(RAYWHITE, 0.30f));

    int start_x = screen_width / 2 - 260;
    int start_y = 220;
    int spacing = 45; // Space between lines

    // Movement
    draw_text_shadow("A / LEFT   - Move Left", start_x, start_y, 26, RAYWHITE);
    draw_text_shadow("D / RIGHT  - Move Right", start_x, start_y + spacing, 26, RAYWHITE);
    
    // Jumping
    draw_text_shadow("SPACE      - Jump", start_x, start_y + spacing * 2, 26, SKYBLUE);

    // Ladders
    draw_text_shadow("W / UP     - Climb Ladder", start_x, start_y + spacing * 4, 26, RAYWHITE);
    draw_text_shadow("S / DOWN   - Descend Ladder", start_x, start_y + spacing * 5, 26, RAYWHITE);

    // System
    draw_text_shadow("R          - Restart Level", start_x, start_y + spacing * 7, 26, GOLD);
    draw_text_shadow("ESC        - Pause / Menu", start_x, start_y + spacing * 8, 26, GOLD);

    draw_centered_text("Press ESC to return", screen_height - 60, 22, Fade(RAYWHITE, 0.75f));
}
